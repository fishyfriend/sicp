...
;(search-for-primes (expt 10 3) false 3) ; timings: 0.20s, 0.20s, 0.21s
;(search-for-primes (expt 10 4) false 3) ; timings: 0.60s, 0.60s, 0.60s
;(search-for-primes (expt 10 5) false 3) ; timings: 1.83s, 1.83s, 1.81s
;(search-for-primes (expt 10 6) false 3) ; timings: 5.84s, 5.84s, 5.85s
...
;(search-for-primes (expt 10 3) false 3) ; timings: 1.79s, 1.87s, 1.93s
;(search-for-primes (expt 10 4) false 3) ; timings: 2.29s, 2.24s, 2.31s
;(search-for-primes (expt 10 5) false 3) ; timings: 2.59s, 2.67s, 2.70s
;(search-for-primes (expt 10 6) false 3) ; timings: 3.03s, 3.01s, 3.18s
...
; (fixed-point f first-guess) would require to converge on a solution, or false
; if it would not converge within max-steps steps.
(define (fixed-point-count-steps f first-guess max-steps)
  (define (close-enough? v1 v2)
    (< (abs (- v1 v2)) tolerance))
  (define (try guess step)
    (let ((next (f guess)))
      (cond ((>= step max-steps) false)
